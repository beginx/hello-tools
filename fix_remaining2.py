#!/usr/bin/env python3
"""Fix all hyphenated keys in layout.js and regenerate scientific-calc"""
import os

# Fix 1: layout.js - quote ALL hyphenated keys
with open('src/app/[locale]/layout.js', 'r', encoding='utf-8') as f:
    content = f.read()

# Find all keys with hyphens and quote them
import re
# Pattern: word-word: (at the start of a line with indentation)
content = re.sub(r'^(\s+)([a-z]+-[a-z]+):', r'\1"\2":', content, flags=re.MULTILINE)

with open('src/app/[locale]/layout.js', 'w', encoding='utf-8') as f:
    f.write(content)

print("Fixed all hyphenated keys in layout.js")

# Fix 2: Regenerate scientific-calc/page.js properly
scientific_calc = '''import { useState, useCallback } from 'react';
...
with open('src/app/[locale]/scientific-calc/page.js', 'w', encoding='utf-8') as f:
    f.write(scientific_calc)

print("Regenerated scientific-calc/page.js")