...
export default function OhmPage() {
  ...
  const calc = () => {
    ...
    if (v !== '' && p !== '' && !isNaN(nV) && !isNaN(nP)) {
      if (nV !== 0) { res.iv2 = nP / nV; res.rv2 = (nV * nV) / nP; }
    }
    if (i !== '' && r !== '' && !isNaN(nI) && !isNaN(nR)) {
      res.vv = nI * nR; res.pv3 = (nI * nI) * nR;
    }
    if (i !== '' && p !== '' && !isNaN(nI) && !isNaN(nP)) {
      if (nI !== 0) { res.vv2 = nP / nI; res.rv3 = nP / (nI * nI); }
    }
      ...
      <div className="os9-window" style={{ maxWidth: 440, width: '100%' }}>
        ...
        <div className="os9-window-body">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div>
              <label className="os9-label">{t('voltage')}</label>
              <input className="os9-input" type="number" step="any" min={0} value={v}
                onChange={(e) => setV(e.target.value)} placeholder="0" />
            </div>
            <div>
              <label className="os9-label">{t('current')}</label>
              <input className="os9-input" type="number" step="any" min={0} value={i}
                onChange={(e) => setI(e.target.value)} placeholder="0" />
            </div>
            <div>
              <label className="os9-label">{t('resistance')}</label>
              <input className="os9-input" type="number" step="any" min={0} value={r}
                onChange={(e) => setR(e.target.value)} placeholder="0" />
            </div>
            <div>
              <label className="os9-label">{t('power')}</label>
              <input className="os9-input" type="number" step="any" min={0} value={p}
                onChange={(e) => setP(e.target.value)} placeholder="0" />
            </div>
          </div>

          <div className="flex gap-2">
            <button className="os9-btn os9-btn-primary flex-1 text-base py-3" onClick={calc}>
              {t('calculate')}
            </button>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(result.vv !== undefined || result.vv2 !== undefined || result.vv3 !== undefined) && (
                  <div className="os9-result text-center">
                    ...
                    <p className="os9-big-number" style={{ fontSize: '1.25rem' }}>
                      {(result.vv || result.vv2 || result.vv3 || 0).toFixed(3)}
                    </p>
                ...
                {(result.iv !== undefined || result.iv2 !== undefined || result.iv3 !== undefined) && (
                  <div className="os9-result text-center">
                    ...
                    <p className="os9-big-number" style={{ fontSize: '1.25rem' }}>
                      {(result.iv || result.iv2 || result.iv3 || 0).toFixed(3)}
                    </p>
                ...
                {(result.rv !== undefined || result.rv2 !== undefined || result.rv3 !== undefined) && (
                  <div className="os9-result text-center">
                    ...
                    <p className="os9-big-number" style={{ fontSize: '1.25rem' }}>
                      {(result.rv || result.rv2 || result.rv3 || 0).toFixed(3)}
                    </p>
                ...
                {(result.pv !== undefined || result.pv2 !== undefined || result.pv3 !== undefined) && (
                  <div className="os9-result text-center">
                    ...
                    <p className="os9-big-number" style={{ fontSize: '1.25rem' }}>
                      {(result.pv || result.pv2 || result.pv3 || 0).toFixed(3)}
                    </p>
