...
# ===== HARDCODED EN METADATA FOR ALL 59 TOOLS =====
# Extracted from expand_en_tools.py + the original layout.js
en_meta = {
...
actual_tools = sorted([
    ...
    if os.path.isfile(os.path.join(PAGE_DIR, item, 'page.js')) and item in en_meta
])

print(f'Tools with EN metadata: {len(actual_tools)}')

# Check coverage
missing_from_meta = [t for t in os.listdir(PAGE_DIR) if os.path.isfile(os.path.join(PAGE_DIR, t, 'page.js')) and t not in en_meta]
if missing_from_meta:
    print(f'WARNING: Tools without metadata: {missing_from_meta}')

# Generate line for each tool
def make_entry(name, desc, cat):
    name_esc = name.replace('"', '\\"').replace('&', '&')
    desc_esc = desc.replace('"', '\\"').replace('&', '&')
    return f'    {tool}: {{ name: "{name_esc}", desc: "{desc_esc}", cat: "{cat}" }},'

# Build tools object for all 5 locales
locale_lines = []
for loc in ['en', 'es', 'zh', 'ko', 'pt']:
    locale_lines.append(f'  {loc}: {{')
    for tool in sorted(actual_tools):
        ...
        if loc != 'en':
            ...
            suffix_map = {
                'es': 'Estilo retro Mac OS 9.',
                'zh': 'Mac OS 9复古风格。',
                'ko': 'Mac OS 9 레트로 스타일.',
                'pt': 'Estilo retro Mac OS 9.',
            }
            prefix = prefix_map.get(loc, '')
            suffix = suffix_map.get(loc, '')
            
            if 'Free online ' in desc:
                desc = desc.replace('Free online ', prefix + ': ', 1)
            if desc.endswith('retro style.'):
                desc = desc.replace('Mac OS 9 retro style.', suffix)
            elif suffix not in desc:
                desc = desc.rstrip('.').rstrip('.') + f'. {suffix}'
        
        name_esc = name.replace('"', '\\"')
        desc_esc = desc.replace('"', '\\"')
        locale_lines.append(f'    {tool}: {{ name: "{name_esc}", desc: "{desc_esc}", cat: "{cat}" }},')
    locale_lines.append('  },')

tools_block = 'const tools = {\n' + '\n'.join(locale_lines) + '\n};'
print(f'Generated {len(actual_tools)} tools × 5 locales')

# Build toolMap
tool_map = '  const toolMap = { ' + ', '.join(f"'{t}': '{t}'" for t in sorted(actual_tools)) + ' };'
print(f'toolMap: {len(actual_tools)} entries')

# Read current layout.js
with open(LAYOUT, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace tools object
import re
old_tools = re.search(r'const tools = \{[^}]*\};', content, re.DOTALL)
if old_tools:
    content = content.replace(old_tools.group(0), tools_block)
    print('Replaced tools object')

# Replace toolMap  
old_map = re.search(r'const toolMap\s*=\s*\{[^}]+\};', content)
if old_map:
    content = content.replace(old_map.group(0), tool_map)
    print('Replaced toolMap')

with open(LAYOUT, 'w', encoding='utf-8') as f:
    f.write(content)

# Verify
final_en = re.findall(r'^\s+(\w+):\s*\{', content.split('en: {')[1].split('es: {')[0], re.MULTILINE)
print(f'\nFinal EN tools: {len(final_en)}')
final_tm = len(re.findall(r"'/(\w+)':\s*'\\w+'", content))
print(f'Final toolMap entries: ~{final_tm}')
